...
namespace CYM
{
    ...
    public class BaseCoreMono : BaseMono, IBase, IMono, IDBMono, IUnit, IOnAnimTrigger
    {
        ...
        List<BaseMgr> needGameLogicTurnComponets = new List<BaseMgr>();
        ...
        public virtual LayerData LayerData => Const.Layer_Default;
        ...
        public override void Awake()
        {
            ...
            if (LayerData != Const.Layer_Default)
                SetLayer(LayerData, true);
            OnSetNeedFlag();
            GlobalMonoMgr.AddMono(this);
            OnAttachComponet();
            OnAffterAwake();
        ...
        public virtual void OnGameFrameTurn(int gameFramesPerSecond)
        {
            ...
            {
                if (value.IsEnable)
                    value.OnGameFrameTurn(gameFramesPerSecond);
            }
        }
        // 手动调用(动画触发)
