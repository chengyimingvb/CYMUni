...
namespace CYM
{
    public static class ExtensionNormal
    {
        ...
        public static string GetDescV<T>(this List<Cost<T>> data) where T : Enum
        {
            ...
            if (costStr != "")
                retStr = BaseLanguageMgr.Get("AC_消耗", UIUtil.Yellow(costStr));
            return retStr;
        }
        ...
        #region attr change
        public static float GetVal<T>(this List<AttrChange<T>> data, T type) where T : Enum
        {
            if (data == null)
            {
                return 0;
        ...
        public static string GetDesc<T>(this List<AttrChange<T>> data, string Separator = "", string EndSeparator = ",") where T : Enum
        {
            string temp = "";
            if (data == null)
            {
            ...
            foreach (var item in data)
            {
                if (item.RealVal == 0)
                    continue;
                temp += Separator + item.GetDesc(true, true) + EndSeparator;
            }
            return temp.TrimEnd(EndSeparator.ToCharArray());
        }
        #endregion
        ...
        {
            return BaseLanguageMgr.Get(Const.Prefix_Desc + GetFull(myEnum), objs);
        }
        /// <summary>
        /// 获得枚举的全名
        /// </summary>
        ...
        {
            return BaseLanguageMgr.Get(str, param);
        }
        public static string GetDesc(this string str, params object[] param)
        {
            return BaseLanguageMgr.Get(Const.Prefix_Desc + str, param);
        }
        ...
        public static void ActiveSelf(this Text text,bool b)
        {
            if (text == null)
                return;
            text.gameObject.SetActive(b);
        }
        public static void ActiveSelf(this Image image,bool b)
        {
            if (image == null)
                return;
