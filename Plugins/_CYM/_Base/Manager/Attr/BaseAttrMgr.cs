...
namespace CYM
{
    ...
    // 属性管理器基类,T必须为属性枚举
    public class BaseAttrMgr<T> : BaseMgr, IBaseAttrMgr 
        where T : Enum
    {
        #region Callback val
        ...
        public virtual void ChangeVal(List<AttrChange<T>> datas)
        {
        ...
        public static string GetAttrNumberStr(T Type, float Val)
        {
            ...
            if (tempAttrData != null)
            {
                ...
                if (tempData.NumberType == NumberType.KMG)
                    ...
                    return UIUtil.OneD(Val, true);
                else if (tempData.NumberType == NumberType.D2)
                    return UIUtil.TwoD(Val,true);
        ...
        // 获得所有属性
        public Dictionary<T, float> GetDBData()
        {
            Dictionary<T, float> ret = new Dictionary<T, float>();

            for (int i = 0; i < _baseDataPool.Length; ++i)
            {
                ret.Add((T)(object)(i), _getBaseAttrVal(i));
            }
            return ret;
        }
