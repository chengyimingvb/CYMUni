...
namespace CYM
{
    ...
    public class BaseBuffMgr<TData, TType> : BaseMgr, IDBListConverMgr<DBBaseBuff>, IBaseBuffMgr 
        ...
        where TType : Enum
    {
        ...
        public virtual TData Add(string buffName)
        {
            if (buffName.IsInv())
                return null;
            if (!TDLuaMgr.Contains(buffName))
            {
                CLog.Error("未找到buff errorId=" + buffName);
                return null;
            }
            TData tempBuff = TDLuaMgr.Get<TData>(buffName).Copy<TData>();
            return Add(tempBuff);
        }
        ...
        public virtual void Remove(string buffName, RemoveBuffType type = RemoveBuffType.Once)
        {
            ...
            if (!TDLuaMgr.Contains(buffName)) return;
        ...
        public List<TData> GetTableBuffs(List<string> ids)
        {
            ...
            {
                var tempBuff = TDLuaMgr.Get<TData>(item);
                if (tempBuff == null)
                {
        ...
        {
            var tempBuff = TDLuaMgr.Get<TData>(id);
            if (tempBuff == null)
            {
        ...
        {
            var tempBuff = TDLuaMgr.Get<TData>(buffId);
            if (tempBuff == null)
            {
        ...
        #region db
        public List<DBBaseBuff> GetDBData()
        {
            List<DBBaseBuff> ret = new List<DBBaseBuff>();
        ...
        public void LoadDBData(List<DBBaseBuff> data)
        {
            if (data == null) return;
            foreach (var item in data)
            {
