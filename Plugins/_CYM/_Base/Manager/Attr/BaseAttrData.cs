...
namespace CYM
{
    ...
    #region attr
    // 属性加成
    public class AttrAdditon<T> : IBase, ICloneable, IBaseAttrAdditon where T : Enum
    {
        ...
        // 真实的数值
        public float RealVal => Mathf.Clamp(AnticipationVal(InputVal), Min, Max);
        ...
        public float AnticipationVal(float? inputVal)
        {
            ...
            if (FactionType == AttrFactionType.PercentAdd) return Val * (1 + faction);
            else if (FactionType == AttrFactionType.DirectAdd) return Val + (faction);
            else if (FactionType == AttrFactionType.DirectMul) return Val * faction;
            else if (FactionType == AttrFactionType.None) return Val;
    ...
    #region data
    public class AttrChange<T> : IBase, ICloneable, IBaseUpFactionData where T : Enum
    {
        #region prop
        public long ID { get; set; }
        public string TDID { get; set; }
        ...
        public float InputValStart { get; set; } = 0;
        public float InputVal { get; private set; }
        ...
        #region set
        public AttrChange<T> SetInputVal(float val)
        {
            InputVal = val;
            return this;
        }
        public AttrChange<T> SetAdd(float val)
        {
            Add = val;
            return this;
        }
        public AttrChange<T> SetPercent(float val)
        {
            Percent = val;
            return this;
        }
    ...
    // 消耗数据结构体
    [Serializable]
    public class Cost<T> : ICloneable, IBaseUpFactionData where T : Enum
    {
        #region construct
        ...
        {
            if (isReserve) return "+" + Type.GetName() + UIUtil.TwoD(RealVal);
            else return "-" + Type.GetName() + UIUtil.TwoD(RealVal);
        }
        #endregion
