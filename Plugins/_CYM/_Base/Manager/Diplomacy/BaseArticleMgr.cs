...
namespace CYM
{
    ...
    public class BaseArticleMgr<TUnit, TWarData, TArticleData> : BaseGFlowMgr, IBaseArticleMgr, IDBListConverMgr<DBBaseArticle>
        ...
        where TArticleData : TDBaseArticleData, new()
    {
        ...
        #region prop
        //谈判模板
        readonly Dictionary<string, BaseNegotiationTemp<TUnit, TWarData>> NegotiationTemps = new Dictionary<string, BaseNegotiationTemp<TUnit, TWarData>>();
        //我方的协议
        ...
        //当前的谈判模板
        public BaseNegotiationTemp<TUnit, TWarData> CurNegotiation { get; protected set; }
        ...
        public BaseNegotiationTemp<TUnit, TWarData> GetNegotiation(string tdid)
        {
        ...
        {
            var data = TDLuaMgr.Get<TArticleData>(articleTDID);
        ...
        #region DB
        public void LoadDBData(List<DBBaseArticle> data)
        {
            data.ForEach((x) =>
            {
                TArticleData article = TDLuaMgr.Get<TArticleData>(x.TDID).Copy<TArticleData>();
                Util.CopyToTD(x, article);
        ...
        public List<DBBaseArticle> GetDBData()
        {
            List<DBBaseArticle> ret = new List<DBBaseArticle>();
            Datas.ForEach((x) =>
            {
                if (x.SelfBaseUnit == null || x.BaseTarget == null)
                    return;
                DBBaseArticle dbItem = new DBBaseArticle();
                Util.CopyToDB(x, dbItem);
