...
namespace CYM
{
    public class BaseRelationMgr<TUnit, TRelationSip, TWarData> : BaseGFlowMgr, IBaseRelationMgr
        where TUnit : BaseUnit
        where TRelationSip : TDBaseRelationShipData, new()
        where TWarData : BaseWarfareData<TUnit>, IBaseWarfareData, new()
    {
        #region Callback val
        ...
        public event Callback<HashList<TUnit>> Callback_OnChangeRelation;
        public event Callback Callback_OnBaseChangeRelation;
        #endregion
        ...
        #region life
        ITDLuaMgr TDRelationShipLuaMgr;
        TDBaseRelationShip<TRelationSip> TDBaseRelationShipLuaMgr;
        protected virtual int ArmisticeCD => 10;
        protected virtual int MaxRelationShip => TDBaseRelationShipData.MaxRelationShip;
        protected virtual int MinRelationShip => TDBaseRelationShipData.MinRelationShip;
        ...
        {
            base.OnAffterAwake();
            TDRelationShipLuaMgr = BaseLuaMgr.GetTDLuaMgr(typeof(TRelationSip));
            if (TDRelationShipLuaMgr == null)
                CLog.Error("错误!没有相关的配置表:{0}", typeof(TRelationSip).GetNiceName());
            TDBaseRelationShipLuaMgr = TDRelationShipLuaMgr as TDBaseRelationShip<TRelationSip>;
            if (TDBaseRelationShipLuaMgr == null)
                CLog.Error("错误!TDRelationShip 必须继承 TDBaseRelationShip");
        }
        public override void OnGameLogicTurn()
        {
            ...
            foreach (var item in Warfare)
                item.OnUpdate();
            //更新停战协议
            List<MultiKey<TUnit>> keys = new List<MultiKey<TUnit>>();
        ...
        public void SetRelationShip(TUnit first, TUnit second, int val)
        {
            if (first == null || second == null) return;
            val = Mathf.Clamp(val, MinRelationShip, MaxRelationShip);
            RelationShip.Change(first, second, val);
            if (BaseGlobal.IsUnReadData)
            {
                OnChangeRelationShip(first, second, val);
            }
        }
        public void ChangeRelationShip(TUnit first, TUnit second, int val)
        {
            if (first == second) return;
            var curVal = RelationShip.Get(first, second);
            RelationShip.Change(first, second, Mathf.Clamp(curVal + val, MinRelationShip, MaxRelationShip));
            if (BaseGlobal.IsUnReadData)
            {
                OnChangeRelationShip(first, second, val);
            }
        ...
        public string GetRelationShipID(int val)
        {
            return TDBaseRelationShipLuaMgr.GetByRange(val)?.TDID;
        }
        public TRelationSip GetRelationShipData(int relationShip)
        {
            return TDBaseRelationShipLuaMgr.GetByRange(relationShip);
        }
        public TRelationSip GetRelationShipData(TUnit first, TUnit second)
        {
            return TDBaseRelationShipLuaMgr.GetByRange(GetRelationShip(first, second));
        }
        public TRelationSip GetRelationShipData(string tdid)
        {
            return TDRelationShipLuaMgr.Get<TRelationSip>(tdid);
        }
        #endregion
        ...
        public T GetWarfareData<T>(long id) where T : class, IBaseWarfareData
        {
            return Warfare.Get(id) as T;
        }
        ...
        {
            Callback_OnChangeRelation?.Invoke(units);
            Callback_OnBaseChangeRelation?.Invoke();
    ...
    #region class
    [Serializable]
    public class BaseWarfareData<TUnit> : IBase, IBaseWarfareData where TUnit : BaseUnit
    {
        #region prop
        ...
        {
            return UIUtil.PerToIntCol(GetWarPointPercent(unit));
        }
        ...
        {
            return BaseLanguageMgr.Get("Text_战争名称", GetAttackerChief().BaseConfig.GetName(), GetDefenserChief().BaseConfig.GetName());
        }
