...
namespace CYM
{
    ...
    public class BaseMgr : IBase, IDBMono, IUnit
    {
        ...
        protected Terrain ActiveTerrain => BaseSceneRoot.ActiveTerrain;
        protected TerrainData ActiveTerrainData => BaseSceneRoot.ActiveTerrain.terrainData;
        ...
        DBBaseGame CurBaseGameData => DBMgr.CurBaseGameData;
        #endregion

        #region wrapper
        public Vector3 Pos => SelfBaseUnit.Pos;
        public Vector3 LocalPos => SelfBaseUnit.LocalPos;
        #endregion
        ...
        // mono的日期帧
        public virtual void OnGameLogicTurn()
        {
            foreach (var item in subComponets)
            {
            ...
            SetDirtyAllAction(true);
        }
        // 帧同步
        public virtual void OnGameFrameTurn(int gameFramesPerSecond)
        {
            foreach (var item in subComponets)
                if (item.IsEnable)
        ...
        protected string JointStr(string key, params object[] ps) => BaseLanguageMgr.Joint(key, ps);
        ...
        #region DB
        public virtual void OnRead1<TDBData>(TDBData data) where TDBData : DBBaseGame, new()
        {
            foreach (var item in subComponets)
                item.OnRead1(data);
        }
        public virtual void OnRead2<TDBData>(TDBData data) where TDBData : DBBaseGame, new()
        {
            foreach (var item in subComponets)
                item.OnRead2(data);
        }
        public virtual void OnRead3<TDBData>(TDBData data) where TDBData : DBBaseGame, new()
        {
            foreach (var item in subComponets)
                item.OnRead3(data);
        }
        public virtual void OnReadEnd<TDBData>(TDBData data) where TDBData : DBBaseGame, new()
        {
            foreach (var item in subComponets)
                item.OnReadEnd(data);
        }
        public virtual void OnWrite<TDBData>(TDBData data) where TDBData : DBBaseGame, new()
        {
            foreach (var item in subComponets)
                item.OnWrite(data);
        }
