...
namespace CYM
{
    ...
    public class BaseTransactionMgr<TUnit> : BaseGFlowMgr, IDBListConverMgr<DBBaseTransact> where TUnit : BaseUnit
    {
        ...
        protected void UpdateTransact()
        {
            List<BaseTransact<TUnit>> clears = new List<BaseTransact<TUnit>>();
            foreach (var item in Data)
            {
                foreach (var item2 in item.Value)
                {
                    ...
                    if (item2.Value.Time.IsOver())
                        clears.Add(item2.Value);
                }
            }
            foreach (var item in Data)
            {
                foreach (var item2 in clears)
        ...
        public List<DBBaseTransact> GetDBData()
        {
            List<DBBaseTransact> ret = new List<DBBaseTransact>();
            foreach (var item in Data)
            {
                foreach (var item2 in item.Value)
                {
