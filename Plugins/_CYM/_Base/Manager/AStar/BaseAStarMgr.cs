...
namespace CYM
{
    ...
    public class BaseAStarMgr : BaseGFlowMgr
    {
        ...
        Coroutineter BattleCoroutine => BaseGlobal.BattleCoroutineter;
        ...
        // 移动单位到一个指定的位置点上面
        public bool SetPosition(Vector3 pos, BaseUnit moveUnit)
        {
            ...
            bool isDistanceWithEnd = true)
        {
            ...
            GraphNode GetCustomNode(Vector3 tempPos)
            {
                ...
                foreach (var item in links)
                {
                    ...
                    if (isDistanceWithEnd) curDistance = MathUtil.XZSqrDistance((Vector3)target.position, (Vector3)item.position);
                    else curDistance = MathUtil.XZSqrDistance(selfUnit.Pos, (Vector3)item.position);
                    if (IsCondition(item))
                    {
                        if (isMaxDistance)
                        {
        ...
        public GraphNode GetClosedNode(BaseUnit selfUnit,int depath=int.MaxValue, Vector3? target = null)
        {
            ...
            GraphNode GetCloset(int range)
            {
                ...
                foreach (var item in nodes)
                {
                    ...
                    //得到距离最近的一个节点
                    var dis_self = MathUtil.XZSqrDistance(selfUnit.Pos, (Vector3)item.position);
        ...
        public float DistanceWithBlocker(BaseUnit withBloderUnit, GraphNode node)
        {
            ...
            {
                float curDistance = MathUtil.XZSqrDistance((Vector3)node.position, (Vector3)item.position);
        ...
        public static void SpeedUp(float mul)
        {
            MultipleSpeed = mul;
        }
        #endregion
