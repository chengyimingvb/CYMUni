...
namespace CYM
{
    ...
    public class BaseHUDMgr<T> : BaseMgr, IBaseHUDMgr 
        where T : UHUDBar
    {
        ...
        protected virtual UHUDView HUDView => throw new NotImplementedException("此函数必须被实现");
        protected virtual UHUDView JumpHUDView => throw new NotImplementedException("此函数必须被实现");
        ...
        public JumpFontData? JumpDamage(string key, [DefaultValue("Color.red")] Color col, bool needPlayer = false, params string[] objs) => JumpDamageStr(BaseLanguageMgr.Get(key, objs), col, needPlayer);
        public JumpFontData? JumpDamage(float val, bool needPlayer = false) => JumpDamageStr(UIUtil.RoundD(val), Color.red, needPlayer);
        public JumpFontData? JumpDamage(string key, bool needPlayer = false, params string[] objs) => JumpDamageStr(BaseLanguageMgr.Get(key, objs), Color.red, needPlayer);
        #endregion
        ...
        public JumpFontData? JumpTreat(string key, [DefaultValue("Color.green")] Color col, bool needPlayer = false, params string[] objs) => JumpTreatStr(BaseLanguageMgr.Get(key, objs), col, needPlayer);
        public JumpFontData? JumpTreat(float val, bool needPlayer = false) => JumpTreatStr(UIUtil.RoundD(val), Color.green, needPlayer);
        public JumpFontData? JumpTreat(string key, bool needPlayer = false, params string[] objs) => JumpDamageStr(BaseLanguageMgr.Get(key, objs), Color.green, needPlayer);
        #endregion
