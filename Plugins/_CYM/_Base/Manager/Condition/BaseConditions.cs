...
namespace CYM
{
    ...
    #region Cost
    // 消耗判断
    public class BaseCostCondition<TUnit, Type> : BaseCondition where Type : Enum where TUnit : BaseUnit
    {
        ...
        public override string GetDesc()
        {
            ...
            foreach (var item in CostDatas)
            {
                ...
                if (MathUtil.Round(GetAttrVal(item.Type), 2) < item.RealVal)
                    tempBool = false;
                string tempstr = BaseLanguageMgr.Get("AC_IsAttrToAct", item.Type.GetName(), ACCompareType.MoreEqual.GetFull().GetName(), item.GetDesc(false, false, false));
        ...
        #region 必须重载的函数
        protected virtual float GetAttrVal(Type type) => throw new NotImplementedException();
        #endregion
    }
    #endregion

    #region Attr
    public class BaseAttrCondition<TUnit, Type> : BaseCondition where Type : Enum where TUnit : BaseUnit
    {
    ...
    public class SimpleCondition
    {
        ...
        public virtual string GetDesc()
        {
            string str = IsAutoTrans ? BaseLanguageMgr.Get(key, objs) : key;
