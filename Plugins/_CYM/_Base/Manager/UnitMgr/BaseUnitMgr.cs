...
namespace CYM
{
        ...
        where TOwner : BaseUnit
    {
        ...
        public virtual void Despawn(TUnit unit)
        {
            ...
            if (!Data.ContainsValue(unit)) return;
            RemoveFromData(unit);
            GMgr.Despawn(unit);
        ...
        protected virtual void OnPostOccupied(TUnit unit)
        {
            CalcAveragePos();
        }
        protected virtual void OnBeOccupied(TUnit unit) { }
        protected virtual void OnBePostOccupied(TUnit unit)
        {
            CalcAveragePos();
        }
        ...
        BaseUnit IUnitMgr.SpawnNew(string id, Vector3 pos, Quaternion? quaternion) => SpawnNew(id, pos, quaternion);
