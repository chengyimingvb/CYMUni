...
namespace CYM
{
    ...
    public class BaseLanguageMgr : BaseGFlowMgr, ILoader
    {
        ...
        public static HashList<string> LoadTips { get; private set; } = new HashList<string>();
        static HashList<LanguageType> LangTypes = new HashList<LanguageType>();
                    ...
                    else
                    {
                        ...
                        for (int colIndex = 0; colIndex < NumberOfCols; ++colIndex)
                        {
                            ...
                            if (colIndex == 0)
                            {
                                ...
                                //跳过无效的字符
                                if (key.IsInv()) continue;
                                //跳过注释
                                if (key.StartsWith(Const.Prefix_Lang_Notes)) continue;
                                //跳过分类
