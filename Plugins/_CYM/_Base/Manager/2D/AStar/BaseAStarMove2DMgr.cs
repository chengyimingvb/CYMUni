...
namespace CYM
{
    ...
    public class BaseAStarMove2DMgr : BaseMgr
    {
        ...
        public AStarGrid AstarGrid => AStarGridManager.DefaultGrid;
        ...
        public AgentDirection MoveDirect { get; private set; } = AgentDirection.Default;
        ...
        public override void OnEnable()
        {
            ...
            MoveDirect = AgentDirection.Default;
            Session = null;
        ...
        protected virtual void OnUpdatePath()
        {
            ...
            switch (State)
            {
                ...
                case AgentState.FollowingPath:
                    ...
                    break;
                case AgentState.AwaitingFollowing:
                    {
        ...
        protected virtual void OnUpdateToNodePos(Vector3 target)
        {
            ...
            if (direct.sqrMagnitude != 0)
            {
                if (direct.y <= 0.002f)
                {
                    MoveDirect = AgentDirection.Forward;
                }
                else
                {
                    MoveDirect = AgentDirection.Backward;
                }

                if (direct.x > 0)
                {
                    MoveDirect |= AgentDirection.Right;
                }
                else if (direct.x < 0)
                {
                    MoveDirect |= AgentDirection.Left;
                }
            }
        ...
        protected void ChangeState(AgentState state)
        {
            if (state == AgentState.AwaitingFollowing)
            {
                AwaitFollowingCount = 0;
                AwaitingFollowingTimer.Restart(AwaitingFollowingTime);
                this.State = state;
