...
namespace CYM
{
    ...
    [Serializable]
    public class TDBaseBuffData<TType> : TDBaseData where TType : Enum
    {
        #region def
        ...
        public string PercentCDStr
        {
            ...
            {
                if (!IsHaveRTMaxTime)
                    return BaseLanguageMgr.Get("Text_永久");
        ...
        // 获得buff的加成描述字符串组合
        public string GetAdtDesc(bool isNewLine = false, string splite = Const.STR_Indent)
        {
            string addition = "";
        ...
        {
            string ret = "";
            ret = string.Format("<b>{0}</b> {1}", GetName(), !IsHaveRTMaxTime ? "" : CurTime.ToString() + "/" + RTMaxTime);
            ret += GetAdtDesc(true, Const.STR_Indent);
            return ret;
        }
        #endregion

        #region must override
        protected virtual BasePerformMgr PerformMgr => null;
        // 属性管理器
        protected virtual BaseAttrMgr<TType> AttrMgr => throw new NotImplementedException("AttrMgr:此函数没有被实现");
        #endregion
    }
