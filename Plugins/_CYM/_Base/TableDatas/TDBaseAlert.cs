...
namespace CYM
{
    ...
    public class TDBaseAlertData : TDBaseData
    {
        ...
        //管理器
        public IBaseAlertMgr AlertMgr { get; set; }
        #endregion
        ...
        //战争
        public IBaseWarfareData WarfareData { get; set; } = null;
        //额外参数
        ...
        public override void OnBeAdded(BaseCoreMono selfMono, params object[] obj)
        {
            ...
            if (TipStr.IsInv())
            {
                if (!TipKey.IsInv()) TipStr = GetStr(TipKey);
                else if (OnGetTip != null) TipStr = OnGetTip?.Invoke(this);
                else TipStr = GetStr(TDID);
            }
            if (DetailStr.IsInv()) DetailStr = OnGetDetail?.Invoke(this);
            if (TitleStr.IsInv()) TitleStr = GetStr(TDID);
        ...
        public Sprite GetIllustration() => GRMgr.Icon.Get(Illustration);
        ...
        public TDBaseAlertData SetAuto(string baseKey, string illustration, string sfx, params object[] descPS)
        {
            ...
            if (!BaseLanguageMgr.IsContain(tipKey))
                tipKey = baseKey;
            SetTitleStr(baseKey);
        ...
        public bool IsValid()
        {
            if (Type == AlertType.Continue) return true;
            if (IsOver()) return false;
            if (OnIsValid == null) return true;
        ...
        public virtual void DoEnter()
        {
            ...
            else BaseTooltipView.Default?.ShowStr(TipStr);
        }
        ...
        public class TDBaseAlert<T> : TDBaseConfig<T> where T : TDBaseAlertData, new() { }
    }
}