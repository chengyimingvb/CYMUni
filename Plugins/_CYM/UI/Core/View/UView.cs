...
namespace CYM.UI
{
    ...
    public class UView : BaseCoreMono, IUIDirty, IDragHandler, IPointerDownHandler
    {
        ...
        protected Color BlockerCol = new Color(0, 0, 0, 0);

        [FoldoutGroup("Prop"), Tooltip("是否忽略Blocker,UIMgr 有个 IsBlockClick 标记,用于屏蔽所有UI的点击事件")]
        ...
        #region life
        protected virtual string FocusClip => "UI_Focus";
        ...
        public override void Awake()
        {
            ...
            if (Canvas != null) CanvasTrans = Canvas.transform as RectTransform;
            ...
            if (IsFullScreen && IsAddBlocker)
            {
                ...
                Blocker.Init(new UImageData { ClickClip = "UI_Error",Color = ()=> BlockerCol });
            }
            if (!IsExclusive)
                IsReturn = false;
        }
        ...
        #region set
        public void IgnoreBlockClick(bool b)
        {
            IsIgnoreBlockClick = b;
        }
        ...
        #region get
        // 获得翻译
        protected static string GetStr(string key, params object[] objs) => BaseLanguageMgr.Get(key, objs);
        protected static string JointStr(string key, params object[] objs) => BaseLanguageMgr.Joint(key,objs);
        protected static string GetACAll() => BaseGlobal.ACM.GetAll();
        #endregion
        ...
        protected virtual bool Inspector_HideGroup()
        {
            return false;
        }
        protected virtual bool Inspector_HideIsExclusive()
        {
            return Group == ViewGroup.None;
        }
        protected virtual bool Inspector_HideIsReturn()
        {
            return Group == ViewGroup.None || !IsExclusive;
        }
        protected virtual bool Inspector_HideAddBlocker()
        {
            return !IsFullScreen;
        }
        protected virtual bool Inspector_HideBlockerCol()
        {
            return !IsAddBlocker || !IsFullScreen;
        }
        #endregion
        ...
        protected virtual void OnSwitchLanguage() => SetDirtyRefresh();
        // 显示状态发生变化的时候调用
