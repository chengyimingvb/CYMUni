...
namespace CYM.UI
{
    ...
    public class UControl : UIBehaviour, IUIDirty
    {
        ...
        #region child
        protected HashList<UControl> DynamicChilds { get; set; } = new HashList<UControl>();
        ...
        public string ACAll { get; protected set; }
        //mgr
        ...
        protected BaseLanguageMgr LangMgr => BaseGlobal.LangMgr;
        ...
        [FoldoutGroup("Base"), SerializeField, Tooltip("默认显示类型")]
        ...
        public bool IsBeFetched { get; protected set; } = false;
        //是否第一次Enable
        ...
        public virtual bool IsCanBeViewFetch => IsIndependent;
        //是否可以被上层父对象获取
        public virtual bool IsCanBeControlFetch => IsIndependent;
        //自动获取子组件
        public virtual bool IsAutoFetchSubControls => true;
        //是否自动初始化
        ...
        protected virtual List<UControl> Childs
        {
            get
            {
                if (DynamicChilds.Count > 0)
        ...
        public virtual void Refresh()
        {
            ...
            if (IsShow)
            {
                foreach (var child in Childs)
                {
                    ...
                    {
                        //CheckBox特殊处理
                        if(child is UCheckBox checkBox)
        ...
        public virtual void RefreshCell()
        {
            ...
            if (IsShow)
            {
                foreach (var child in CellChilds)
                {
                    child.RefreshCell();
                }
            }
